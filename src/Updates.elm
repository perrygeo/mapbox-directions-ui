...
calcBounds features =
    let
        lngs =
            List.map (\f -> f.lng) features

        lats =
            List.map (\f -> f.lat) features
    in
...
updateDestinations model newDestinations =
    let
        runSearch =
            List.length newDestinations > 1

        bbox =
            calcBounds newDestinations
        ...
        cmds =
            ...
            , if runSearch then
                ...
                MapboxGl.routesToMap <| List.map routeFeatureObject []
              -- clear route
            ]
    in
...
moveItem item relPos data =
    let
        pos =
            List.Extra.elemIndex item data
...
update msg model =
    case Debug.log "Update Message:" msg of
        ...
        AddDestination feature ->
            let
                newDestinations =
                    List.append model.destinations (List.singleton feature)
            in
                updateDestinations model newDestinations
        ...
        MoveDestination feature spaces ->
            let
                newDestinations =
                    moveItem feature spaces model.destinations
            in
                updateDestinations model newDestinations
